...
public class AuctionProductController {
    ...
    public AuctionProductResponse publishProduct(@RequestBody AuctionProductRequest request) {
        try {
            ...
            if ("Guest".equalsIgnoreCase(request.getSellerUsername())
                    || !userDao.existsByUsername(request.getSellerUsername())) {
                return new AuctionProductResponse(false, "Vui lòng đăng nhập trước khi đăng sản phẩm.");
            }

            if (userDao.isAdmin(request.getSellerUsername())) {
                return new AuctionProductResponse(false, "Admin không thể đăng sản phẩm.");
            }
    ...
    public AuctionProductResponse removeProduct(@PathVariable int productId, @RequestParam String adminUsername) {
        try {
            if (productId <= 0 || isBlank(adminUsername)) {
                return new AuctionProductResponse(false, "Thông tin xóa sản phẩm không hợp lệ.");
            }

            AuctionProductResponse response = auctionProductDao.removeProduct(productId, adminUsername);
