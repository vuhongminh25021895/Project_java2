...
public class SwitchSceneController {
    public static Scene loadScene(SceneName sceneName) throws IOException {
        FXMLLoader loader = new FXMLLoader(SwitchSceneController.class.getResource(getFXMLPath(sceneName)));
    ...
    private static String getFXMLPath(SceneName sceneName) {
        switch (sceneName) {
            ...
            case BID:
                return "/BiddingScene.fxml";
            case AUCTION_LIST:
                return "/AuctionList.fxml";
            case PRODUCT_PUBLISHING:
                return "/ProductPublishing.fxml";
            case NAP_TIEN:
                return "/NapTien.fxml";
            default:
