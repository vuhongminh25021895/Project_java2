...
public class AuctionProductService {
    private static final String BASE_URL = "http://100.64.15.22:8080/api/products";
    ...
    public AuctionProductResponse placeBid(int productId, BidRequest bidRequest) {
        String json = gson.toJson(bidRequest);
        RequestBody body = RequestBody.create(json, MediaType.get("application/json"));
        Request request = new Request.Builder()
                .url(BASE_URL + "/" + productId + "/bid")
                .post(body)
                .build();
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            return new AuctionProductResponse(response.isSuccessful(),
                    response.isSuccessful() ? "Đặt giá thành công." : "Đặt giá thất bại.");
        } catch (IOException e) {
            return new AuctionProductResponse(false, "Không thể kết nối tới server.");
        }
    }
    ...
    public AuctionProductResponse removeProduct(int productId, String adminUsername) {
        HttpUrl url = HttpUrl.parse(BASE_URL + "/" + productId)
                .newBuilder()
                .addQueryParameter("adminUsername", adminUsername)
                .build();
        Request request = new Request.Builder()
                .url(url)
                .delete()
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            return new AuctionProductResponse(response.isSuccessful(),
                    response.isSuccessful() ? "Đã xóa sản phẩm." : "Xóa sản phẩm thất bại.");
        } catch (IOException e) {
            return new AuctionProductResponse(false, "Không thể kết nối tới server.");
        }
    }

    public List<BidHistoryPoint> fetchBidHistory(int productId) throws IOException {
        Request request = new Request.Builder()
                .url(BASE_URL + "/" + productId + "/history")
                .get()
                .build();
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            Type listType = new TypeToken<List<BidHistoryPoint>>() { }.getType();
            List<BidHistoryPoint> history = gson.fromJson(responseBody, listType);
