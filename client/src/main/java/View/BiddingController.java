...
public class BiddingController {
    ...
    private void applySuccessfulBid(AuctionProductResponse response) {
        ...
        applyBidUpdate(currentPrice, response.getStatus(), response.getBidTime());
    }

    private void updateBidControls() {
        boolean isOpen = product != null && "OPEN".equalsIgnoreCase(product.getStatus());

        btnGiaToiThieu.setDisable(!isOpen);
        btnDangGia.setDisable(!isOpen);
        txtGiaMoi.setDisable(!isOpen);
    ...
    private void handleBidUpdate(BidUpdateEvent event) {
        ...
        Platform.runLater(() -> applyBidUpdate(event.getCurrentPrice(), event.getStatus(), event.getBidTime()));
    }

    private void applyBidUpdate(Integer currentPrice, String status, String bidTime) {
